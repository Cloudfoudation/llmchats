// src/components/knowledge/ShareKnowledgeBaseModal.tsx
...
export const ShareKnowledgeBaseModal: React.FC<ShareKnowledgeBaseModalProps> = ({
    isOpen,
    onClose,
    knowledgeBase
}) => {
    ...
    // Initialize sharing states and visibility when groups load
    useEffect(() => {
        if (knowledgeBase.knowledgeBaseId) {
            // Set initial visibility based on knowledgeBase data
            setVisibility(knowledgeBase.visibility as 'private' | 'public' || 'private');

            if (groups.length > 0) {
                const initialStates: SharingState = {};
                const sharedResources = getResourceGroups(knowledgeBase.knowledgeBaseId, 'knowledge-base');
    ...
    }, [groups, knowledgeBase, getResourceGroups]);
    ...
    const handleVisibilityChange = async (newVisibility: 'public' | 'private') => {
        ...
        try {
            ...
            if (success) {
                ...
                throw new Error(`Failed to set knowledge base visibility to ${newVisibility}`);
            }
        } catch (error) {
            setLocalError(error instanceof Error ? error.message : `Failed to set knowledge base visibility to ${newVisibility}`);
        } finally {
    ...
    const handleShare = async (group: Group) => {
        ...
        try {
            ...
            if (success) {
                ...
                throw new Error('Failed to share knowledge base');
            }
        } catch (error) {
            setLocalError(error instanceof Error ? error.message : 'Failed to share knowledge base');
            setSharingStates(prev => ({
                ...prev,
                [group.groupId]: {
                    ...prev[group.groupId],
    ...
    const handleUnshare = async (group: Group) => {
        ...
        try {
            const success = await unshareResource(knowledgeBase.knowledgeBaseId, group.groupId);
            ...
            if (success) {
                ...
                throw new Error('Failed to unshare knowledge base');
            }
        } catch (error) {
            setLocalError(error instanceof Error ? error.message : 'Failed to unshare knowledge base');
            setSharingStates(prev => ({
                ...prev,
                [group.groupId]: {
                    ...prev[group.groupId],
    ...
    const handlePermissionChange = async (group: Group, permission: keyof SharingState[string]['permissions'], value: boolean) => {
        if (!knowledgeBase.knowledgeBaseId || !sharingStates[group.groupId]?.isShared) return;
        ...
        try {
            await updateSharedResource(knowledgeBase.knowledgeBaseId, group.groupId, {
                permissions: newPermissions
            });
        } catch (error) {
            ...
            <div className="flex items-center justify-center min-h-screen p-4">
                ...
                <div className="relative bg-white dark:bg-gray-800 rounded-lg w-full max-w-2xl p-6 max-h-[90vh] overflow-y-auto">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            <h3 className="text-lg font-medium text-gray-900 dark:text-white">Share Knowledge Base</h3>
                            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                Share "{knowledgeBase.name}" with others
                            </p>
                        </div>
                    ...
                    <div className="mb-6">
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                            {visibility === 'private'
                                ? 'Only you and group members you share with can access this knowledge base.'
                                : 'Anyone in your organization can access and use this knowledge base.'}
                        </p>
                    </div>
                    ...
                    {groupsLoading ? (
                        ...
                        <div className="text-center py-8">
                            ...
                            <p className="text-sm text-gray-400 dark:text-gray-500">
                                Create or join groups to share knowledge bases with others.
                            </p>
                        </div>
                    ) : (
                                            ...
                                            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                                                ...
                                                <div className="space-y-2">
                                                    <label className="flex items-center">
                                                        ...
                                                        />
                                                        <span className="ml-2 text-sm text-gray-700 dark:text-gray-300">Can edit and add documents</span>
                                                    </label>
                                                    <label className="flex items-center">
                                                        <input
                                                            type="checkbox"
                                                            ...
                                                            className="rounded border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-blue-600 focus:ring-blue-500 dark:focus:ring-blue-400"
                                                        />
                                                        <span className="ml-2 text-sm text-gray-700 dark:text-gray-300">Can delete documents</span>
                                                    </label>
                                                    <label className="flex items-center">
                                                        <input
                                                            type="checkbox"
